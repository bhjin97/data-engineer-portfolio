...
export function ProjectCard({ project, number }: ProjectCardProps) {
  ...
  return (
    <article className="grid overflow-hidden rounded-2xl border border-slate-200 border-l-blue-500 bg-white shadow-sm transition-[border-color,box-shadow,transform] duration-200 motion-safe:hover:-translate-y-px hover:border-blue-200 hover:shadow-md motion-reduce:transform-none sm:grid-cols-[minmax(0,3fr)_minmax(0,2fr)] lg:min-h-[390px]">
      ...
      <aside className={`hidden bg-slate-50/70 p-8 sm:block sm:border-l sm:border-slate-200 lg:p-9 ${isMetaPipeline ? "" : "lg:flex lg:flex-col"}`} aria-label={isMetaPipeline ? "프로젝트 핵심 검증 결과" : "프로젝트 핵심 기여"}>
